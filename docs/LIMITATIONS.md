...
Scalar metrics (EPS global, IOI CV, synchrony fraction, burstiness) are likewise **score-derived model outputs**, not perceptual measures. VD4 IOI CV and burstiness use **fused onsets** (effective τ); raw diagnostics and Mustextu rates are documented separately. See **[METRIC_SEMANTICS.md](METRIC_SEMANTICS.md)**.
