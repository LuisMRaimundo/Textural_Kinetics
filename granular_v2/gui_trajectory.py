...
class _SampleEditDialog(tk.Toplevel):
...
class TrajectoryTab:
    """Registral trajectory (VD10) tab: editable picks and multi-block analysis."""
    ...
    def samples(self) -> List[Dict[str, float]]:
        """Active block samples (legacy compatibility)."""
    ...
    def _on_numeric_edit_sample(self, _event: tk.Event) -> None:
        ...
        if idx is None:
            return
        dlg = _SampleEditDialog(self._sample_list, self.samples[idx])
    ...
    def _export(self) -> None:
        ...
        export_vd10_session_json(self._session_result, fp)
