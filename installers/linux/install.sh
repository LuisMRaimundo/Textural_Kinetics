#!/usr/bin/env bash
# Granularity Analyser — Linux one-click installer
...
echo ""
echo " Granularity Analyser — Linux installer"
...
step "Writing START-Granularity.sh launcher..."
cat > "$PROJECT_ROOT/START-Granularity.sh" << 'LAUNCHER'
#!/usr/bin/env bash
cd "$(dirname "$0")"
...
LAUNCHER
chmod +x "$PROJECT_ROOT/START-Granularity.sh"

step "Starting Granularity Analyser (GUI window should open)..."
echo "To run again later, run: ./START-Granularity.sh"
echo ""
